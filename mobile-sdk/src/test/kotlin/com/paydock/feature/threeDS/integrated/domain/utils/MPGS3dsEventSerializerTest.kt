...
internal class MPGS3dsEventSerializerTest {
    ...
    fun `test 3ds data serializer with chargeAuthSuccess`() {
        ...
        // Act
        val result = eventJson.convertToDataClass<MPGS3dsEvent>()

        // Assert
        assertIs<MPGS3dsEvent.ChargeAuthSuccessEvent>(result)
        assertEquals(IntegratedEvent.CHARGE_AUTH_SUCCESS, result.event)
        assertEquals(MPGS3dsStatus.AUTHENTICATED, result.data.status)
    ...
    fun `test 3ds data serializer with chargeAuthReject`() {
        ...
        // Assert
        assertIs<MPGS3dsEvent.ChargeAuthRejectEvent>(result)
        assertEquals(IntegratedEvent.CHARGE_AUTH_REJECT, result.event)
        assertEquals(MPGS3dsStatus.NOT_AUTHENTICATED, result.data.status)
    ...
    fun `test 3ds data serializer with chargeAuth`() {
        ...
        // Assert
        assertIs<MPGS3dsEvent.ChargeAuthEvent>(result)
