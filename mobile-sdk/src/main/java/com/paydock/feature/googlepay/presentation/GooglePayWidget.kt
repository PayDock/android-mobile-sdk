...
import com.paydock.core.DEFAULT_ANIMATION_DURATION
...
fun GooglePayWidget(
    ...
    val allowedPaymentMethods: JSONArray? = runCatching {
        ...
        completion(
            Result.failure(
                GooglePayException(stringResource(id = R.string.error_googlepay_payment_methods))
            )
        )
    ...
    val resolvePaymentForResult = rememberLauncherForActivityResult(
        ...
        if (!walletToken.isNullOrBlank()) {
            handleGooglePayActivityResult(result, walletToken, viewModel, completion)
        } else {
            completion(
                Result.failure(
                    GooglePayException(
        ...
        Box(contentAlignment = Alignment.Center) {
            Column(
                ...
                AnimatedVisibility(
                    ...
                    enter = fadeIn(animationSpec = tween(DEFAULT_ANIMATION_DURATION)),
...
private fun handleGooglePayUiStateErrors(
    ...
    if (error != null && error is ErrorModel.GooglePayError) {
...
private fun handleUiStateErrors(
    ...
    error?.let {
        onGooglePayResult(Result.failure(GooglePayException(it.displayableMessage)))
    }
}
...
private fun handleGooglePayActivityResult(
    ...
    when (result.resultCode) {
        ...
        ComponentActivity.RESULT_CANCELED -> {
            ...
            onGooglePayResult(Result.failure(GooglePayException("Google Pay payment has been cancelled")))
        }

        else -> {
            onGooglePayResult(Result.failure(GooglePayException("An unexpected error has occurred trying to use Google Pay")))
        }
    }
}
...
private fun handleGooglePayPaymentResult(
    ...
    runCatching {
        ...
        if (!googleToken.isNullOrBlank()) {
            ...
            onGooglePayResult(Result.failure(GooglePayException("Google Pay Token Error")))
        }
    }.onFailure {
